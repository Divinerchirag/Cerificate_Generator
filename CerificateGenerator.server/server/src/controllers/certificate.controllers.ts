...
export const generateCertificate = async (
  ...
  try {
    const { template_id, student_name, course_name, completion_date } =
      req.body;
    ...
    if (!template_id || !student_name || !course_name || !completion_date) {
      res
        ...
        .json({
          error:
            "template_id, student_name, course_name, and completion_date are required.",
        });
      return;
    }
    ...
    if (errorMessage.includes("Restricted")) {
      ...
      res
        .status(500)
        .json({ error: errorMessage || "Failed to generate certificate." });
    }
  }
};
  ...
  try {
    ...
    if (errorMessage.includes("Restricted")) {
      ...
      res
        .status(500)
        .json({
          error: errorMessage || "Failed to generate bulk certificates.",
        });
    }
  }
};
