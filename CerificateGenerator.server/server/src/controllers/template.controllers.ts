...
export const saveTemplateFields = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    const insertedFields = await templateService.saveTemplateFields(
      id,
      req.user!.id,
      fields,
    );
...
export const updateTemplateFields = async (
  ...
  try {
    ...
    const result = await templateService.updateTemplateFields(
      id,
      req.user!.id,
      fields,
      canvas_width,
    ...
    if (error.message === "Template not found") {
      ...
      res
        .status(500)
        .json({ error: `Failed to update fields: ${error.message}` });
    }
  }
};
