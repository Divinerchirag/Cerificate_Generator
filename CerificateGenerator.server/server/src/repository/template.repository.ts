...
export const deleteTemplateFields = async (
  templateId: string,
): Promise<void> => {
...
export const getCertificatesByTemplateId = async (
  templateId: string,
): Promise<any[]> => {
  const result = await pool.query(
...
export const deleteCertificatesByTemplateId = async (
  templateId: string,
): Promise<void> => {
  await pool.query("DELETE FROM certificates WHERE template_id = $1", [
    templateId,
  ]);
};
