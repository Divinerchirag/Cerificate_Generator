...
export const generateSingleCertificate = async (
  ...
  if (role === "admin") {
    throw new Error(
      "Admins are restricted from generating certificates. Only Users can generate certificates.",
    );
  }

  const templateDetails = await templateRepo.getTemplateById(
    templateId,
    userId,
  );
  if (!templateDetails) {
    ...
    fs.createReadStream(csvFilePath)
      ...
      .on("data", (row: any) => {
        students.push({
          ...
          completion_date:
            row["Completion Date"] || row.completion_date || row.date,
        });
      })
      .on("end", resolve)
      .on("error", reject);
...
export const getCertificatePdfPath = async (id: string) => {
  ...
  return {
    path: pdfFullPath,
    filename: `certificate_${cert.student_name}.pdf`,
  };
};
